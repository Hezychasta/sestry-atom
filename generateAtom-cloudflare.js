...
async function fetchAuthors(apiToken, collectionId, cmsLocaleId) {
  ...
  // Paginacja: pobieraj rekordy partiami po 100, aż nie będzie więcej danych
  while (true) {
    const response = await fetch(
    ...
    if (data.items.length < limit) break; // Koniec, jeśli zwrócono mniej niż limit
    offset += limit;
  }

  return allItems.reduce((map, item) => {
    map[item.id] = item.fieldData.name || `Autor ID: ${item.id}`; // Fallback
    return map;
  }, {});
}
...
async function handleRequest(request) {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const hasPolishContent =
      /[aćęłńóśźż]/i.test(content) && !/[іїєґ]/i.test(content);
...
<feed xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/" xml:lang="pl">
  ...
  <entry>
    ...
    authorIds.forEach((authorId) => {
      const authorName = authorsMap[authorId] || `Autor ID: ${authorId}`; // Fallback
      console.log(`Mapping author ${authorId} to ${authorName}`); // Debugowanie
      xml += `
    <author>
    ...
    const imageCredits = imageCreditsRaw.replace(/<[^>]+>/g, "").trim() || ""; // Usuwa HTML
    if (leadImage) {
      xml += `
    ...
    const contentCleaned = contentRaw
      .replace(/<p[^>]*>/g, "<p>") // Usuwa atrybuty w <p> (np. id="")
      .replace(/<\/?[^>]+(?!p|strong|em)[^>]*>/g, "") // Usuwa wszystkie znaczniki oprócz <p>, <strong>, <em>
      .trim();
    xml += `
